...
import { SignUpWithJWT } from '@/APIs/AuthJWT';
...
export default function SignupPage() {
  ...
  const OnSignUpFormSubmit = async(name:string, email:string, password:string, confirmPassword:string)=>{
    if(password != confirmPassword) throw new Error("Password and confirm password doesnt match");
      ...
      <Card className="w-full max-w-md shadow-2xl">
        ...
        <CardContent className="space-y-6">
          <form  className="space-y-4">
            ...
            <Button type="submit" className="w-full" onClick={()=>{OnSignUpFormSubmit(formData.name, formData.email, formData.password, formData.confirmPassword).catch(err=>alert(err.message))}}  >
              Sign Up
            </Button>
          </form>
